...
import { doaCategories } from "@/config/doa";

export const ALL_DOA_CATEGORY_SLUG = "semua";

export interface DoaCategoryFilterProps {
  selected: string;
...
export function DoaCategoryFilter({
  selected,
  onSelect,
  showFavoritesOnly,
  onToggleFavoritesOnly,
  className,
}: DoaCategoryFilterProps) {
  return (
    <div className={cn("flex gap-2 overflow-x-auto scrollbar-none pb-1", className)}>
      <Chip active={selected === ALL_DOA_CATEGORY_SLUG} onClick={() => onSelect(ALL_DOA_CATEGORY_SLUG)}>
        Semua
      </Chip>
